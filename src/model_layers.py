...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model: int, nhead: int, nhead_low: int, dim_feedforward: int = 2048,
                 dropout: float = 0.1, kernel_low: int = 5, kernel_high: int = 11,
                 dilation_low: int = 1, dilation_high: int = 5, avgPoolK=5, avgPoolS=1, time_compression=1,
                 activation: Union[str, Callable[[Tensor], Tensor]] = F.relu,
        ...
        # self.lowAttKVConv = nn.Conv1d(in_channels=d_model, out_channels=d_model, dilation=dilation_low,
        #                                kernel_size=kernel_low, padding=((kernel_low-1)* dilation_low)//2, **factory_kwargs)
        self.lowAttKVConv = nn.AvgPool2d(kernel_size=(avgPoolK, time_compression),
                                         stride=(1, time_compression),
                                         padding=(avgPoolK//2,time_compression//2))
        self.lowAttKVConv.to(factory_kwargs['device'])
        # self.highAttConv = nn.Conv1d(in_channels=d_model, out_channels=d_model, dilation=dilation_high,
        #                               kernel_size=kernel_high, padding=((kernel_high-1)* dilation_high)//2, **factory_kwargs)

        self.mlp = nn.Sequential(OrderedDict([
                                             ('Linear1', nn.Linear(in_features=d_model*2, out_features=d_model, **factory_kwargs)),
                                             ('Activation1', nn.Tanh()),
                                            ])
                                )
        ...
        self.attn2 = AttentionLayer(Attn(False, factor, attention_dropout=dropout, output_attention=output_attention, **factory_kwargs), 
                        d_model, nhead_low, mix=False, **factory_kwargs)
    ...
    def forward(self, x: Tensor, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        x1 = self.high_attention(x)           
        x1 = self.conv_informer(x1)
        x2 = self.low_attention(x)
        x2 = self.conv_informer2(x2)
        cat = torch.cat((x1, x2), dim=0)
        # out = self.mlp(cat)
        return cat
    
    #self attention block
    def high_attention(self, x:Tensor):

        # x = self.highAttConv(x.permute(1,2,0)) # --> [batch, dim, len]
        # x = x.permute(2, 0, 1) # --> [len, batch, dim]
    ...
    def low_attention(self, x: Tensor):
        k  = self.lowAttKVConv(x.permute(1,2,0).reshape((x.shape[1], 2, x.shape[2]//2, x.shape[0]))) # --> [batch, 2, nfft, L/hop+1]
        # k  = self.lowAttKVConv(x.permute(1,2,0))
        k = k.flatten(1,2).permute(2, 0, 1) # --> [len, batch, dim]
        # k = k.permute(2, 0, 1) # --> [len, batch, dim]
        v = k

        x_n = self.attn2(x, k, v, attn_mask=None)
    ...
    def conv_informer2(self, x:Tensor):
            x = self.downConv(x.permute(0,2,1)) # --> [len, dim, batch]
            x = self.norm(x)
            x = self.activation2(x)
